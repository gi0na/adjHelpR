...
.get_unfolded_nodes_vertical <- function(node.labels, ts){
  paste(rep(node.labels,max(ts)),rep(0:(max(ts)), each=length(node.labels)), sep = '_')
}
.get_unfolded_nodes_horizontal <- function(node.labels, ts){
  nodes <- paste(rep(node.labels,each=max(ts)+1),0:(max(ts)), sep = "_")
}

# internal function to return unfolded adjacency
.get_unfolded_adjacency_internal <- function(edge.list, sparse, node.labels, ts, vertical){
  if(vertical){
    nodes <- .get_unfolded_nodes_vertical(node.labels, ts)
  } else{
    nodes <- .get_unfolded_nodes_horizontal(node.labels, ts)
  }
...
.get_unfolded_adjacency <- function(edge.list, node.labels, vertical, sparse){
  # construct unfolded edge list
  unfolded.edge.list <- as_tibble(
    t(apply(edge.list, 1, function(row)
      paste(row[1:2], (-1:0)+as.integer(row[3]), sep = '_'))), .name_repair = ~c('source', 'target'))

  return(
    # construct unfolded adjacency matrix from unfolded edgelist
    .get_unfolded_adjacency_internal(unfolded.edge.list, sparse, node.labels, edge.list$timestamp, vertical = vertical)
  )
}
...
.rename_edgelist <- function(edge.list, select_cols){
  ...
  edge.list %>%
    ...
    mutate(timestamp = .data$timestamp - min(.data$timestamp) + 1)
}
...
.get_unfolded_layout <- function(edge.list, node.labels, vertical, resize_ratio, enlarge_ratio){
  ...
  width <- ifelse(vertical, length(node.labels), max(edge.list$timestamp)+1)
  height <- ifelse(vertical, max(edge.list$timestamp)+1, length(node.labels))
